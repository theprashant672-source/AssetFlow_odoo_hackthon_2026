    ...
    <section className="glass-panel soft-shadow rounded-2xl p-4 sm:p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-3xl">
          ...
          <h1 className="mt-4 text-2xl font-black tracking-tight text-slate-900">{title}</h1>
        ...
        <button className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#9A528D] px-3.5 py-2.5 text-sm font-bold text-white shadow-[0_18px_30px_rgba(154,82,141,0.26)] transition hover:brightness-110">
          <IconPlus size={16} />
          Create entry
        </button>
      </div>
      ...
      <div className="mt-6 grid gap-3 lg:grid-cols-3">
        {[
          ...
          <div key={item.label} className="rounded-2xl border border-slate-200 bg-white/80 p-4 shadow-sm">
            ...
            <div className="mt-1.5 text-2xl font-black tracking-tight text-slate-900">{item.value}</div>
          </div>
        ))}
      </div>

      <div className="mt-6 rounded-2xl border border-dashed border-slate-300 bg-white/50 p-6 text-center text-sm leading-6 text-slate-600">
