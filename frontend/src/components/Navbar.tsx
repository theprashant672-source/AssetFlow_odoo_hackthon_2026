...
export default function Navbar({
  ...
  return (
    <nav className="sticky top-0 z-20 flex w-full items-center justify-between gap-4 bg-[#9c4f96] px-6 py-3 font-sans text-white shadow-sm">
      ...
      <div className="flex flex-wrap gap-6 flex-1 ml-8 min-w-0">
        {NAV_LINKS.map((link) => (
          <a
            key={link.href}
            href={link.href}
            className={`whitespace-nowrap py-1.5 text-sm no-underline hover:text-white transition-colors ${
      ...
      <div className="flex shrink-0 items-center gap-4">
        <button
          type="button"
          className="cursor-pointer whitespace-nowrap rounded-md border-none bg-white px-3.5 py-2 text-sm font-semibold text-[#9c4f96] hover:bg-[#f3e8f2] transition-colors"
        ...
        >
          🔔
          {unreadCount > 0 && (
            <span className="absolute -right-1.5 -top-1 rounded-full bg-red-600 px-1.5 py-px text-[10px] font-bold leading-tight text-white">
        ...
        <div className="relative flex items-center gap-3 ml-2" ref={profileRef}>
          <a href="/login" className="flex items-center gap-1.5 text-white no-underline text-sm font-semibold px-3.5 py-2 rounded-md hover:bg-white/15 transition-colors">
          ...
          <a href="/signup" className="flex items-center gap-1.5 bg-white text-[#9c4f96] no-underline text-sm font-semibold px-4 py-2 rounded-md shadow-sm hover:bg-[#f3e8f2] hover:-translate-y-px hover:shadow-md transition-all">
          ...
          >
            <span className="flex h-7 w-7 items-center justify-center rounded-full bg-white text-sm font-semibold text-[#9c4f96]">
          ...
          {isProfileOpen && (
            <div className="absolute right-0 top-[calc(100%+8px)] z-10 min-w-[180px] overflow-hidden rounded-lg border border-gray-200 bg-white shadow-lg">
              <div className="flex flex-col gap-1 border-b border-gray-200 px-4 py-3">
                ...
                <span className="self-start rounded-full bg-[#f3e8f2] px-2 py-px text-[0.7rem] font-semibold text-[#9c4f96]">
                  {userRole}
                </span>
              </div>
              <a
